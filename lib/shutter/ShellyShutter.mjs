...
export default class ShellyShutter {
  constructor(manager, name, nodes) {
    ...
    if (nodes.length == 1) {
      this.single = true;
      attrs.push(new Attribute('roller', 0, 100));
    } else {
      this.single = false;
      const angle = new Attribute('angle', -90, 90);
      attrs.push(new Attribute('position', 0, 100, angle));
      attrs.push(angle);
    }
    attrs.forEach(attr => this.attrs[attr.name] = attr);
  ...
  update(attr, level, moving) {
    ...
    {
      const previous = attrInfo.target;
      attrInfo.target = level;
